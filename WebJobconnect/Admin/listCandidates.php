...
<body>
    ...
    ?>
        <div class="d-flex align-items-center justify-content-center">
            <p class="text-primary font-weight-bold">Danh sách này có <?php echo mysqli_num_rows($res); ?> ứng viên</p>
        </div>
    <div class="container-fluid my-5">
        <div class="row">
            <?php while ($row=mysqli_fetch_array($res)){ ?>
            <div class="col-sm-3 d-flex align-content-center justify-content-center">
                <div class="card my-5 border-info" style="width:18rem;border:3px solid;border-radius:25px;overflow:hidden;">
                    <img src="/WebJobconnect/Candidate/image/<?php echo $row['Avatar']; ?>" class="card-img-top border-info" style="border-bottom:3px solid;height:200px;">
                    <div class="card-body">
                        <h5 class="card-title"><?php echo $row['Hovaten']; ?></h5>
                        <p class="card-text"><?php echo $row['Tentruong']; ?></p>
                        <p class="card-text"><?php echo $row['Nganhhoc']; ?></p>
                        <div class="d-flex align-content-center justify-content-center">
                            <a href="/Admin/home/danh-sach-ung-vien/chi-tiet-ung-vien/<?php echo $row['IDUngvien']; ?>" class="btn btn-primary mx-2 p-2 flex-grow-1 rounded-pill">Thông tin chi tiết</a>
                            <script>
                                function confirmDelete(id){
                                if (confirm("Bạn có chắc chắn muốn xóa ứng viên này ra khỏi hệ thống?")){
                                    window.location.href="/Admin/home/xu-li-xoa-ung-vien/" + id;
                                    }
                                }
                            </script>
                            <a href="#" onclick="confirmDelete('<?php echo $row['IDUngvien']; ?>');" class="btn btn-danger rounded-pill">Xóa</a>
                        </div>
                    </div>
                </div>
            </div>
